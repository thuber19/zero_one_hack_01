...
def build_id2cat(tokenizer):
    """token id -> physical-category index (specials/pad -> -1). Derives the
    next-category targets for the aux head WITHOUT touching the data pipeline."""
...
def train_model(
    ...
    unk_dropout: prob. of replacing a real (id>=7) context token with [UNK] — the
    OOD lever (forces context reliance), targets untouched.
    id2cat: if given (transformer only), adds the next-CATEGORY auxiliary loss so
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            if unk_dropout > 0.0:                       # OOD lever: mask context to [UNK]
...
def main():
    ...
    id2cat_t, n_categories = None, 0
    if args.aux_category and args.arch == "transformer":
        id2cat_list, n_categories = build_id2cat(tokenizer)
        id2cat_t = torch.tensor(id2cat_list, dtype=torch.long)
        print(f"  Auxiliary category head ON ({n_categories} categories, weight={args.cat_weight})")
    ...
    if args.init_from and Path(args.init_from).exists():
        ...
        print(f"  Continued from {args.init_from} (missing={len(missing)}, unexpected={len(unexpected)})")

    # record size so inference loads the matching architecture
    (OUTPUT_DIR / "model_config.json").write_text(json.dumps({"model_size": args.model_size}))
