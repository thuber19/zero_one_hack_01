...
#SBATCH --time=2:00:00
#SBATCH --job-name=process_seq_train
...
# Install pixi environment on compute node if needed
$HOME/.pixi/bin/pixi install --manifest-path "$PROJECT_DIR/pixi.toml"

# Construct run command to execute inside pixi environment
export RUN_COMMAND="$HOME/.pixi/bin/pixi run --as-is --manifest-path $PROJECT_DIR/pixi.toml"
