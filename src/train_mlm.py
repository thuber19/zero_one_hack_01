...
def _load_sequences(data_dir: Path, splits_path: Path, tokenizer: MLMTokenizer,
                    max_len: int, debug: bool, rank: int = 0) -> dict[str, list[list[int]]]:
    ...
    # Bug-1 guard: only trust splits.json if it actually has a non-empty train split.
    # A stale empty file (written by a previous failed run) is treated as missing.
    _splits_valid = False
    if splits_path.exists():
        with open(splits_path) as f:
            splits_data = json.load(f)
        if splits_data.get("train"):
            _splits_valid = True
    ...
    if not _splits_valid:
        # Build splits from scratch; overwrite any stale/missing splits.json.
        # Bug-2 fix: only rank 0 writes, then barrier so all ranks see a complete file.
...
def main() -> int:
    ...
    if vocab_path.exists():
        ...
        # Bug-3 fix: use same CSV-discovery order as _load_sequences (large > variants)
        _data_dir = Path(args.data_dir)
        _csv_candidates = {
            "IC":    [_data_dir / "IC_large.csv",    _data_dir / "IC_variants.csv"],
            "IGBT":  [_data_dir / "IGBT_large.csv",  _data_dir / "IGBT_variants.csv"],
            "MOSFET":[_data_dir / "MOSFET_large.csv", _data_dir / "MOSFET_variants.csv"],
        }
        _csv_for_vocab = {
            v: next(p for p in ps if p.exists())
            for v, ps in _csv_candidates.items()
            if any(p.exists() for p in ps)
        }
        tokenizer = MLMTokenizer.build(_csv_for_vocab)
        ...
        if is_main(rank):
            ...
            # Bug-4 fix: create vocab.json at the expected Spec-001 path so subsequent runs
            # skip the rebuild. Use a symlink; fall back to copy if the filesystem forbids it.
            if not vocab_path.exists():
                vocab_path.parent.mkdir(parents=True, exist_ok=True)
                try:
                    vocab_path.symlink_to(fallback)
                except OSError:
                    import shutil as _shutil
                    _shutil.copy2(fallback, vocab_path)
    ...
    split_seqs = _load_sequences(
        Path(args.data_dir), Path(args.splits), tokenizer, max_len, args.debug, rank
    )
