...
# Stage variant CSVs from the repo's training_data directory
mkdir -p "$TMPDIR/fab_sequences"
TRAINING_DATA="$REPO_DIR/tracks/industrial-infineon/training_data"
cp "$TRAINING_DATA/IC_variants.csv"    "$TMPDIR/fab_sequences/"
cp "$TRAINING_DATA/IGBT_variants.csv"  "$TMPDIR/fab_sequences/"
cp "$TRAINING_DATA/MOSFET_variants.csv" "$TMPDIR/fab_sequences/"
# Also copy the large CSVs if present (generated by generate_data.py)
cp "$TRAINING_DATA/IC_large.csv"       "$TMPDIR/fab_sequences/" 2>/dev/null || true
cp "$TRAINING_DATA/IGBT_large.csv"     "$TMPDIR/fab_sequences/" 2>/dev/null || true
cp "$TRAINING_DATA/MOSFET_large.csv"   "$TMPDIR/fab_sequences/" 2>/dev/null || true
# Copy splits.json from $WORK if already created by a previous run
